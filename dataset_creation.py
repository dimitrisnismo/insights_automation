...
def create_df(start, end, numberofrowsperproduct):
    ...
    df_DSLR["Manufacturer"] = np.random.choice(
        ["Nikon", "Canon", "GoPro", "Sony"], len(df_DSLR)
    )
    df_DSLR["Revenue"] = np.random.randint(350, 700, len(df_DSLR))
    df_DSLR["Amount"] = np.where(np.random.randint(1, 10, len(df_DSLR)) <= 2, 1, 2)
    ...
    df_smart["Amount"] = np.where(np.random.randint(1, 10, len(df_smart)) <= 3, 1, 2)
    col = "Amount"
    ...
    df_TV = df_temp.copy()
    df_TV["Product"] = "TV"
    df_TV["Manufacturer"] = np.random.choice(
        ["Samsung", "LG", "Sony", "Xiaomi", "FU", "TOSHIBA", "PANASONIC"], len(df_TV)
    )
    df_TV["Revenue"] = np.random.randint(700, 1900, len(df_TV))
    df_TV["Amount"] = np.where(np.random.randint(1, 10, len(df_TV)) <= 7, 1, 2)

    # Laptop Fake Dataset
    df_Laptop = df_temp.copy()
    df_Laptop["Product"] = "Laptop"
    df_Laptop["Manufacturer"] = np.random.choice(
        ["Lenovo", "Dell", "HP", "Apple", "Huawei", "Acer", "Other"], len(df_Laptop)
    )
    df_Laptop["Revenue"] = np.random.randint(500, 1500, len(df_Laptop))
    df_Laptop["Amount"] = np.where(np.random.randint(1, 10, len(df_Laptop)) <= 6, 1, 2)

    # Gaming Fake Dataset
    df_Gaming = df_temp.copy()
    df_Gaming["Product"] = "Gaming"
    df_Gaming["Manufacturer"] = np.random.choice(
        ["Sony", "Microsoft", "Nintendo", "Steam"], len(df_Gaming)
    )
    df_Gaming["Revenue"] = np.random.randint(250, 600, len(df_Gaming))
    df_Gaming["Amount"] = np.where(np.random.randint(1, 10, len(df_Gaming)) <= 4, 1, 2)
